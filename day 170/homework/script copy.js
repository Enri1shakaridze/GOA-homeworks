...
async function screenproducts() {
    ...
    resdata.forEach(product => {
        ...
        pbtn.addEventListener('click', function () {
            if (array.includes(divall.innerHTML.replace('<button id="pbtn">კალათაში დამატება</button>', '<button id="rbtn">კალათიდან წაშლა</button>'))) {
                alert('ესეთი პროდუქტი უკვე დამატებულია!');
                return;
            }
            array.push(divall.innerHTML.replace('<button id="pbtn">კალათაში დამატება</button>', '<button id="rbtn">კალათიდან წაშლა</button>'));
            console.log(array);
            res1.innerHTML = array.join('')
            ln = array.length;
            let rbtnm = document.querySelectorAll('#rbtn');
            rbtnm.forEach(remove => {
                remove.addEventListener('click', function () {

                    let parent = remove.parentElement.parentElement;
                    console.log(parent)

                    array = array.filter(item => item !== parent);
                    res1.innerHTML = array.join('');
                    ln = array.length;
